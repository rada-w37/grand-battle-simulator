...
test("provides semantic structure for the static application shell", () => {
  ...
  assert.match(indexHtml, /style\.css\?v=20260811-ios-map-export/);
  assert.match(indexHtml, /js\/main\.js\?v=20260811-aura-composite/);
...
test("uses modal confirmation for destructive actions and keeps map export labels visible", () => {
  ...
  assert.match(mapExportSource, /getMapLayoutCssVars\(MAP_BASE_WIDTH\)/);
  assert.match(mapExportSource, /applyDesktopMapCssVariables\(root\)/);
  assert.match(mapExportSource, /const mapImageSource = mapImageElement\?\.getAttribute\("src"\) \|\| ""/);
  assert.match(mapExportSource, /mapImageElement\?\.remove\(\)/);
  assert.match(mapExportSource, /context\.drawImage\(backgroundImage, 0, 0, MAP_BASE_WIDTH, MAP_BASE_HEIGHT\)/);
  assert.match(mapExportSource, /createSvgDataUrl\(root, exportStyles, "map-export-aura-pass"\)/);
  assert.match(mapExportSource, /createSvgDataUrl\(root, exportStyles, "map-export-overlay-pass"\)/);
  assert.match(mapExportSource, /context\.globalCompositeOperation = "soft-light"/);
  assert.match(mapExportSource, /context\.drawImage\(auraImage, 0, 0, MAP_BASE_WIDTH, MAP_BASE_HEIGHT\)/);
  assert.match(mapExportSource, /context\.globalCompositeOperation = "source-over"/);
  assert.match(mapExportSource, /context\.drawImage\(overlayImage, 0, 0, MAP_BASE_WIDTH, MAP_BASE_HEIGHT\)/);
  assert.match(mapExportSource, /return "data:image\/svg\+xml;base64," \+ btoa/);
  assert.match(mapExportSource, /map-export-aura-pass \.point-aura\{mix-blend-mode:normal!important;\}/);
  assert.match(mapExportSource, /map-export-overlay-pass \.point-aura\{display:none!important;\}/);
  ...
  assert.match(mainSource, /api\.js\?v=20260811-aura-composite/);
  assert.match(mainSource, /ui\.js\?v=20260811-aura-composite/);
  assert.match(mainSource, /events\.js\?v=20260811-aura-composite/);
  assert.match(eventsSource, /api\.js\?v=20260811-aura-composite/);
  assert.match(eventsSource, /ui\.js\?v=20260811-aura-composite/);
  assert.match(worldSelectorSource, /api\.js\?v=20260811-aura-composite/);
  assert.match(guildNameEditorSource, /ui\.js\?v=20260811-aura-composite/);
  assert.match(occupationTabsSource, /ui\.js\?v=20260811-aura-composite/);
  assert.match(occupationTabsSource, /battle-data-dialog\.js\?v=20260811-aura-composite/);
  assert.match(uiSource, /worldSelector\.js\?v=20260811-aura-composite/);
  assert.match(uiSource, /guildNameEditor\.js\?v=20260811-aura-composite/);
  assert.match(uiSource, /occupationTabs\.js\?v=20260811-aura-composite/);
  assert.match(uiSource, /battle-data-dialog\.js\?v=20260811-aura-composite/);
  assert.match(uiSource, /map-export\.js\?v=20260811-aura-composite/);
  ...
  assert.match(styleSource, /\.map-score-panel\.is-collapsed \{[\s\S]*?width: 26px;[\s\S]*?border-radius: 5px;/);
